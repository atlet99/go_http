...
void main() {
  group('GoHttpClient.request', () {
    test('returns the response on success', () async {
      final transport = FakeTransport([ok(200, Uint8List.fromList([1, 2]))]);
    ...
    test('error interceptors run exactly once per attempt', () async {
      final transport = FakeTransport([status(503), status(503)]);
      ...
      // Two attempts => error interceptor runs exactly twice (once each),
      // never twice for the same attempt.
      expect(counter.onErrorCount, 2);
      client.dispose();
    });
    ...
    test('cancellation is not retried and throws CancellationError', () async {
      ...
      final client = GoHttpClient(
        transport: transport,
        // A policy that WOULD retry network errors — must not retry cancellation.
        retryPolicy: DefaultRetryPolicy(
          maxAttempts: 5,
          baseDelay: Duration.zero,
          maxDelay: Duration.zero,
  ...
  group('GoHttpClient auth refresh + retry', () {
    ...
    test('does not loop forever when refresh keeps failing', () async {
      ...
      // Initial attempt + one auth retry = 2 transport calls; refresh runs once.
      expect(transport.callCount, 2);
  ...
  group('GoHttpClient convenience methods', () {
    final cases = <String,
        Future<Response<Uint8List>> Function(GoHttpClient, Uri)>{
